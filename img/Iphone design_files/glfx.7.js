// IMPORTANT: This file has been modified by Canva.
...
');
      ...
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(4), gl.STREAM_DRAW);
...
');
  ...
  exports.canvas = function() {
    var canvas = document.createElement('canvas');
    var config = { premultipliedAlpha: false };
    try {
...
void main() {\
  gl_FragColor = texture2D(texture, texCoord);\
    ...
    function Shader(vertexSource, fragmentSource) {
      ...
      // highp precision is an optional part of the OpenGL ES 2.0 spec
      // so this provides a fallback for devices that don't have highp floats
...
');
  ...
  function channels(r, g, b) {
    gl.channels = gl.channels || new Shader(null, '\
uniform sampler2D texture;\
uniform float r;\
uniform float g;\
uniform float b;\
...
void main() {\
  vec4 color = texture2D(texture, texCoord);\
  if (r > 0.0) {\
    color.r += (255.0 - color.r) * r/100.0;\
  } else if (r < 0.0) {\
    color.r -= color.r * abs(r/100.0);\
  }\
  if (g > 0.0) {\
    color.g += (255.0 - color.g) * g/100.0;\
  } else if (g < 0.0) {\
    color.g -= color.g * abs(g/100.0);\
  }\
  if (b > 0.0) {\
    color.b += (255.0 - color.b) * b/100.0;\
  } else if (b < 0.0) {\
    color.b -= color.b * abs(b/100.0);\
  }\
  \
  gl_FragColor = color;\
}\
');
    ...
    simpleShader.call(this, gl.channels, {
      r: clamp(-1, r, 1),
      g: clamp(-1, g, 1),
      b: clamp(-1, b, 1)
    });
...
void main() {\
  vec4 color = texture2D(texture, texCoord);\
  color.r = pow(color.r/255.0, adjust)*255.0;\
  color.g = pow(color.g/255.0, adjust)*255.0;\
  color.b = pow(color.b/255.0, adjust)*255.0;\
  \
  gl_FragColor = color;\
}\
');
